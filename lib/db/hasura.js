...
export async function addReservation(token, tables, issuer, date, currentDate) {
  const operationsDoc = `
  mutation reservations($tables: String!, $issuer: String!, $date: String!, $currentDate: String!) {
    insert_booking(objects: {bookingDate: $date, bookingTables: $tables, bookingOrderDate: $currentDate, userId: $issuer}) {
      returning {
        id
      }
    }
