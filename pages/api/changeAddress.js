import { changeAddressQuery } from "../../lib/db/hasura";

export default async function changeAddress(req, res) {
  if (req.method === "POST") {
    try {
      ...
      const newAddress = req ? JSON.parse(req.headers.body).newAddress : null;
      const changes = await changeAddressQuery(
        token,
        newAddress,
        displayName,
        profilePic,
